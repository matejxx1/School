...
def get_year(s):
    try:
           if int(str(s)[-4:])<3000 and int(str(s)[-4:])>1950:
               return int(str(s)[-4:])
...
COLUMNS = [
    # Identifying fields
     "ResponseName", "ReleaseDate","ReleaseYear",
    ...
    "ControllerSupport",
    "IsFree",
     "PurchaseAvail", "SubscriptionAvail",
...
def steam_spy_read(appid, *field_names):
    ...
    if not steam_spy_read.steam_spy:
        with open("steamspy.json", encoding="utf-8") as inp:
...
def record(raw):
    ...
    return {
        ...
        "ReleaseYear" : num(get_year(str(data.get('release_date', {}).get('date', '')))),
...
def main():
    ...
    with open("games-features.csv", "w",encoding="utf-8") as basefile:
